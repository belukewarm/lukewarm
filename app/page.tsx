...
import { ArrowUpRight, Mail, Play, Sparkles, Instagram, Twitter, Send, MoveRight, Laptop, Clapperboard, Wand2, Stars, Check, Settings, LogOut, Plus, Trash2, Save, Upload, Download, Link as LinkIcon, Database } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";

// (The rest of the page is identical to your canvas version for parity)
// I’m inlining the exact component you approved, trimmed only where imports differ.
// ——————————————————————————————————————————————

// Lightweight store + DEFAULT_CONTENT from canvas (shortened comment)
...
function loadFromLocal() {
  try {
    ...
    if (!fromLS) return DEFAULT_CONTENT;
    const parsed = JSON.parse(fromLS);
    return { ...DEFAULT_CONTENT, ...parsed };
  } catch { return DEFAULT_CONTENT; }
}
function saveToLocal(data: any) { localStorage.setItem(LS_KEY, JSON.stringify(data)); }

async function supaLoad(cfg: any) {
...
async function supaSave(cfg: any, data: any) {
...
const ContentCtx = React.createContext<any>({
  data: DEFAULT_CONTENT, setData: (_:any)=>{}, authed:false, setAuthed: (_:boolean)=>{},
  syncing:false, setSyncing: (_:boolean)=>{}, syncNow: async ()=>{},
});
...
function Section({ id, eyebrow, title, children }:{ id?:string; eyebrow?:string; title?:string; children:any }){
  return (<section id={id} className="relative z-10 mx-auto max-w-6xl px-4 py-20 md:py-28">
...
function Nav(){ return (<div className="sticky top-6 z-40 mx-auto max-w-6xl px-4">
  <div className="flex items-center justify-between rounded-full border border-neutral-200/60 bg-white/70 px-3 py-2 shadow-sm backdrop-blur-md dark:border-neutral-800/60 dark:bg-neutral-900/60">
    <a href="#home" className="inline-flex items-center gap-2 text-sm font-medium text-neutral-900 dark:text-neutral-100">
      <div className="h-6 w-6 rounded-full bg-neutral-900 dark:bg-neutral-100" /><span>Luke</span>
    </a>
    ...
    <div className="flex items-center gap-2">
      <Button asChild={true} size="sm" variant="ghost" className="rounded-full">
        <a href="#contact" className="inline-flex items-center gap-1"><Mail className="h-4 w-4"/><span>Reach out</span></a>
      </Button>
    </div>
  </div>
</div>);}
...
function Hero(){
  ...
  const ySlow = useTransform(scrollY, [0, 600], [0, -20]);
  const ySlower = useTransform(scrollY, [0, 600], [0, -40]);
  const [open, setOpen] = useState(false);
  const vid = youtubeIdFromUrl(data.hero.reelUrl);
    ...
    <div className="grid grid-cols-1 items-end gap-10 md:grid-cols-12">
      <motion.div style={{ y: ySlow }} className="md:col-span-7">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          {vid && (<Button onClick={()=>setOpen(true)} variant="secondary" className="rounded-full"><Play className="mr-1 h-4 w-4"/>Watch reel</Button>)}
        </div>
...
function AdminPanel(){
  const { data, setData, setAuthed, setSyncing } = useContent();
...
function AdminGate(){
  ...
  useEffect(()=>{
    ...
    const onKey = (e: KeyboardEvent)=>{ if (e.shiftKey && e.key.toLowerCase()==='l') setShowLogin((s)=>!s); };
  ...
  return (<div className="fixed inset-0 z-[55] grid place-items-center bg-black/30 p-4 backdrop-blur-sm">
    {!authed ? (<div className="w-full max-w-sm rounded-2xl border border-neutral-200 bg-white p-5 shadow-xl dark:border-neutral-800 dark:bg-neutral-900">
      ...
      <div className="flex items-center justify-between">
        <Button variant="ghost" className="rounded-full" onClick={()=>setShowLogin(false)}>Cancel</Button>
...
function Footer(){ return null; } // not used here (we defined above)

export default function Page(){
  const [data, setData] = useState<any>(loadFromLocal());
  const [authed, setAuthed] = useState<boolean>(false);
  const [syncing, setSyncing] = useState<boolean>(false);
  useEffect(()=>{ (async()=>{ const cfg=data.admin?.supabase; if(cfg?.enabled && cfg?.url && cfg?.anonKey){ setSyncing(True as any); const remote = await supaLoad(cfg); setSyncing(False as any); if(remote){ setData(remote); saveToLocal(remote);} } })(); saveToLocal(data);},[]);
